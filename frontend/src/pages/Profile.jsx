...
const Profile = () => {
    ...
    let [frontendImage, setFrontendImage] = useState(userData?.user?.image || dp);
    ...
    if (userData?.user) {
      ...
      setFrontendImage(userData?.user?.image || dp);
    }
  }, [userData]);
  ...
  return (
    ...
    items-center justify-center gap-[20px]'>
        ...
        relative' onClick={()=>image.current.click()}>
            ...
            <div className='absolute bottom-4 right-4 text-gray-700 w-[35px] h-[35px]
            rounded-full bg-[#20c7ff] flex justify-center items-center shadow-gray-300 shadow-lg'> 
                <IoCameraOutline className='text-gray-700 w-[25px] h-[25px]'/>
            </div>

        </div>
        <form className='w-[95%] max-w-[500px] flex
            flex-col gap-[20px] items-center justify-center' onSubmit={handleProfile}>
        <input type="file" accept='image/*' ref={image} hidden onChange={handleImage} />
            <input type="text" placeholder='Enter your name' className='w-[90%] h-[50px]
            ...
            <input type="text" readOnly className='w-[90%] h-[50px]
                outline-none border-2 border-[#20c7ff] px-[20px] py-[10px] bg-white
                rounded-lg shadow-gray-300 shadow-lg text-gray-400 text-[19px]' value={userData?.user?.userName}/>

            <input type="email" readOnly className='w-[90%] h-[50px]
                outline-none border-2 border-[#20c7ff] px-[20px] py-[10px] bg-white
                rounded-lg shadow-gray-300 shadow-lg text-gray-400 text-[19px]' value={userData?.user?.email}/>
